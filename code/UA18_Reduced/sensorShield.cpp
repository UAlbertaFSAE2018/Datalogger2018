...
int *SensorShield::getSensorValues(){
    for(int i = 0; i < 0b1111; i++){
        ...
        sensorValues[(i * 0b1111)*2] = analogRead(ANALOG0); // writes first half of array
        sensorValues[(i * 0b1111)*2+1] = analogRead(ANALOG1); // writes second half of array

        /*for(int j = 0; j < MUX_SIZE; j++){
            sensorValues[(i * MUX_SIZE + j)*2] = analogRead(ANALOG0); // writes first half of array
            sensorValues[(i * MUX_SIZE + j)*2+1] = analogRead(ANALOG1); // writes second half of array
        }*/
    }
    return sensorValues;
}
